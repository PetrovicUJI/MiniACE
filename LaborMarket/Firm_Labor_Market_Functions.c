...
* \brief: Firm post vacancies second round.
...
{
	//wage will increase from 1-5% on yearky level, thus divided by 12 we get the monthly level.
	WAGE_OFFER = WAGE_OFFER*(1+((random_int(1,5)/100)/12));
...
/* \fn: int Firm_read_job_applications_and_send_job_offers_2()
 
* \brief: Firm read job applications and send job offers second round.
...
{	
	...
	START_JOB_APPLICATIONS_2_MESSAGE_LOOP
	
		add_Job_application(&job_application_list,
		job_applications_2_message->employee_id,
		job_applications_2_message->reservation_wage);
		
    FINISH_JOB_APPLICATIONS_2_MESSAGE_LOOP
	...
	for(i = 0; i < (job_application_list.size); i++)
    {
        add_job_offer_2_message(ID, job_application_list.array[i].employee_id, WAGE_OFFER);
    }
...
/* \fn: int Firm_hire_new_workers_2()
 
* \brief: Firm hire new workers. Read job acceptance message second round.
...
{	
	START_JOB_ACCEPTANCE_2_MESSAGE_LOOP
	
		add_Employee(&EMPLOYEES, 
		job_acceptance_2_message->employee_id,
		job_acceptance_2_message->wage);
		
		DELTA_LABOR_REQUIREMENT--;
		
    FINISH_JOB_ACCEPTANCE_2_MESSAGE_LOOP
